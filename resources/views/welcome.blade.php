...
@section('content')
    ...
    <div class="container text-center">
        ...
        <h3 class="mt-3">
            @auth
            Welcome, {{ Auth::user()->username }}! @else
            Welcome, Guest!
            @endauth
        </h3>
    </div>
    ...
    <div class="container text-center">
        <p class="mt-5">Check out the latest memes below:</p>
                            ...
                            <div class="card-body">
                                ...
                                @if (Auth::check())
                                    <form action="{{ route('like') }}" method="post" class="d-inline">
                                        ...
                                        <button type="submit" class="btn btn-outline-warning" style="padding: 5px 10px; border-radius: 20px;"><span style="font-size: 1.2em;">😆</span></button>
                                    </form>
                                @endif
                                <!-- Total Likes -->
                                <span class="ms-2"><i class="bi bi-heart-fill"></i> <strong>{{ $meme->likes()->count() }}</strong> Hahas</span>
                            </div>
                        </div>
                                ...
                                <div class="card mb-3">
                                    <div class="card-body">
                                        <form action="{{ route('comment') }}" method="post">
                                            ...
                                            <button type="submit" class="btn btn-outline-dark" style="padding: 5px 15px; border-radius: 20px;">Comment</button>
                                        </form>
                                    </div>
                                </div>
                            @endif
