                        ...
                        <div class="card-body">
                            ...
                            @if ($comment->user->id == Auth::id())
                                <form id="deleteCommentForm{{ $comment->id }}"
                                    action="{{ route('deleteComment') }}" method="post" style="position: absolute; right: 10px; bottom: 10px;">
                                    @csrf
                                    <input type="hidden" name="comment_id" value="{{ $comment->id }}">
                                    <i class="ri-delete-bin-6-line delete-icon" onclick="commentDeleteForm({{ $comment->id }})"></i>
                                </form>
                            @endif
                        </div>
                    </div>
