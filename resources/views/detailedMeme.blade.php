                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        @if ($meme->user->id != Auth::id())
                            ...
                            <div>
                                ...
                                <form id="reportMemeForm{{ $meme->id }}"
                                    action="{{route('report')}}" method="get">
                                    @csrf
                                    <input type="hidden" name="meme_id" value="{{ $meme->id }}">
                                    <i id="reportMemeIcon" class="ri-flag-line custom-icon"
                                        onclick="reportMemeForm({{ $meme->id }})" style="cursor: pointer;"></i>
                                </form>
                            </div>
                        @endif
                    </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            @if ($meme->user->id == Auth::id())
                                <div>
                                    ...
                                    <form id="editMemeForm{{ $meme->id }}"
                                        action="{{ route('editMemeView', ['id' => $meme->id]) }}" method="get">
                                        @csrf
                                        <input type="hidden" name="meme_id" value="{{ $meme->id }}">
                        ...
                        <div class="comment card mb-3">
                            <div class="card-body">
                                <div class="justify-content-between align-items-center">
                                    ...
                                    {{-- commented on --}}
                                    <span style="font-size: 0.7em">Commented On:
                                        {{ $comment->created_at->timezone('Asia/Dhaka')->format('M d, Y h:i A') }}
                                    </span>
                                </div>
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
